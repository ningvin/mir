...
static void patterns_init (gen_ctx_t gen_ctx) {
  ...
  VARR_CREATE (int, pattern_indexes, memctl, 0);
  for (i = 0; i < n; i++) {
  ...
  VARR_CREATE (insn_pattern_info_t, insn_pattern_info, memctl, 0);
...
static void target_init (gen_ctx_t gen_ctx) {
  ...
  VARR_CREATE (uint8_t, result_code, memctl, 0);
  VARR_CREATE (int, insn_pattern_indexes, memctl, 0);
  VARR_CREATE (uint64_t, const_pool, memctl, 0);
  VARR_CREATE (const_ref_t, const_refs, memctl, 0);
  VARR_CREATE (label_ref_t, label_refs, memctl, 0);
  VARR_CREATE (uint64_t, abs_address_locs, memctl, 0);
  VARR_CREATE (MIR_code_reloc_t, relocs, memctl, 0);
  VARR_CREATE (call_ref_t, gen_ctx->target_ctx->call_refs, memctl, 0);
...
static void target_finish (gen_ctx_t gen_ctx) {
  ...
  MIR_free (memctl, gen_ctx->target_ctx);
