...
static void ppc64_push_func_desc (MIR_memctl_t memctl, VARR (uint8_t) * *insn_varr);
void (*ppc64_func_desc) (MIR_memctl_t memctl, VARR (uint8_t) * *insn_varr) = ppc64_push_func_desc;

static void ppc64_push_func_desc (MIR_memctl_t memctl, VARR (uint8_t) * *insn_varr) {
  VARR_CREATE (uint8_t, *insn_varr, memctl, 128);
...
void *_MIR_get_bstart_builtin (MIR_context_t ctx) {
  ...
  ppc64_push_func_desc (ctx->memctl, &code);
...
void *_MIR_get_thunk (MIR_context_t ctx) { /* emit 3 doublewords for func descriptor: */
  ...
  VARR_CREATE (uint8_t, code, ctx->memctl, 128);
...
void _MIR_redirect_thunk (MIR_context_t ctx, void *thunk, void *to) {
  VARR (uint8_t) * code;
  VARR_CREATE (uint8_t, code, ctx->memctl, 256);
...
void *_MIR_get_bb_thunk (MIR_context_t ctx, void *bb_version, void *handler) {
  ...
  VARR_CREATE (uint8_t, code, ctx->memctl, 64);
