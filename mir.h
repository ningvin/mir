...
extern MIR_context_t _MIR_init (MIR_memctl_t memctl, MIR_gen_memctl_t gen_memctl);

/* Use only either the following API to create MIR code... */
static inline MIR_context_t MIR_init2 (MIR_memctl_t memctl, MIR_gen_memctl_t gen_memctl) {
  ...
  return _MIR_init (memctl, gen_memctl);
}
...
extern MIR_memctl_t MIR_get_memctl (MIR_context_t ctx);
...
extern void _MIR_set_code (MIR_gen_memctl_t memctl, size_t prot_start, size_t prot_len,
