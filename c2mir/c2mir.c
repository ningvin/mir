...
static inline MIR_memctl_t get_memctl (c2m_ctx_t c2m_ctx) {
  return MIR_get_memctl (c2m_ctx->ctx);
}

static void *reg_malloc (c2m_ctx_t c2m_ctx, size_t s) {
  MIR_memctl_t memctl = get_memctl (c2m_ctx);
  void *mem = MIR_malloc (memctl, s);
...
static void reg_memory_pop (c2m_ctx_t c2m_ctx, size_t mark) {
  ...
  while (VARR_LENGTH (void_ptr_t, reg_memory) > mark)
    MIR_free (memctl, VARR_POP (void_ptr_t, reg_memory));
}
...
static void reg_memory_init (c2m_ctx_t c2m_ctx) {
  MIR_memctl_t memctl = get_memctl (c2m_ctx);
  VARR_CREATE (void_ptr_t, reg_memory, memctl, 4096);
}
...
static void str_init (c2m_ctx_t c2m_ctx) {
  MIR_memctl_t memctl = get_memctl (c2m_ctx);
  HTAB_CREATE (tab_str_t, str_tab, memctl, 1000, str_hash, str_eq, NULL);
  HTAB_CREATE (tab_str_t, str_key_tab, memctl, 200, str_key_hash, str_key_eq, NULL);
...
static void *c2mir_calloc (c2m_ctx_t c2m_ctx, size_t size) {
  MIR_memctl_t memctl = get_memctl (c2m_ctx);
  void *res = MIR_calloc (memctl, 1, size);
...
void c2mir_init (MIR_context_t ctx) {
  ...
  *c2m_ctx_ptr = c2m_ctx = MIR_calloc (memctl, 1, sizeof (struct c2m_ctx));
...
static void init_streams (c2m_ctx_t c2m_ctx) {
  MIR_memctl_t memctl = get_memctl (c2m_ctx);
  cs = eof_s = NULL;
  VARR_CREATE (stream_t, streams, memctl, 32);
}
...
static stream_t new_stream (MIR_memctl_t memctl, FILE *f, const char *fname, int (*getc_func) (c2m_ctx_t)) {
  stream_t s = MIR_malloc (memctl, sizeof (struct stream));

  VARR_CREATE (char, s->ln, memctl, 128);
  s->f = f;
...
static void add_stream (c2m_ctx_t c2m_ctx, FILE *f, const char *fname,
  ...
  cs = new_stream (memctl, f, fname, getc_func);
...
static void set_string_stream (c2m_ctx_t c2m_ctx, const char *str, pos_t pos,
  ...
  cs = new_stream (memctl, NULL, NULL, NULL);
  VARR_PUSH (stream_t, streams, cs);
  cs->pos = pos;
  if (transform != NULL) {
    transform (str, cs->ln);
...
static void init_macros (c2m_ctx_t c2m_ctx) {
  ...
  VARR_CREATE (macro_t, macros, memctl, 2048);
  HTAB_CREATE (macro_t, macro_tab, memctl, 2048, macro_hash, macro_eq, NULL);
  /* Standard macros : */
  ...
  if (!c2m_options->pedantic_p) {
    VARR_CREATE (token_t, params, memctl, 1);
...
static macro_call_t new_macro_call (MIR_memctl_t memctl, macro_t m, pos_t pos) {
  ...
  mc->repl_pos = 0;
  mc->args = NULL;
  VARR_CREATE (token_t, mc->repl_buffer, memctl, 64);
  return mc;
}
...
static void pre_init (c2m_ctx_t c2m_ctx) {
  ...
  VARR_CREATE (char_ptr_t, once_include_files, memctl, 64);
  VARR_CREATE (token_t, temp_tokens, memctl, 128);
  VARR_CREATE (token_t, output_buffer, memctl, 2048);
  init_macros (c2m_ctx);
  VARR_CREATE (ifstate_t, ifs, memctl, 512);
  VARR_CREATE (macro_call_t, macro_call_stack, memctl, 512);
}
...
static void define (c2m_ctx_t c2m_ctx) {
  ...
  id = t;
  t = get_next_pptoken (c2m_ctx);
  VARR_CREATE (token_t, repl, memctl, 64);
  params = NULL;
  if (t->code == '(') {
    VARR_CREATE (token_t, params, memctl, 16);
...
static void find_args (c2m_ctx_t c2m_ctx, macro_call_t mc) { /* we have just read a parenthesis */
  ...
  m = mc->macro;
  VARR_CREATE (token_arr_t, args, memctl, 16);
  VARR_CREATE (token_t, arg, memctl, 16);
...
static void process_directive (c2m_ctx_t c2m_ctx) {
  ...
  VARR_CREATE (token_t, temp_buffer, memctl, 64);
...
static struct val eval_expr (c2m_ctx_t c2m_ctx, VARR (token_t) * expr_buffer, token_t if_token) {
  ...
  VARR_CREATE (token_t, temp_buffer, memctl, VARR_LENGTH (token_t, expr_buffer));
...
static macro_call_t try_param_macro_call (c2m_ctx_t c2m_ctx, macro_t m, token_t macro_id) {
  ...
  mc = new_macro_call (memctl, m, macro_id->pos);
...
#endif
      mc = new_macro_call (memctl, m, t->pos);
...
static void tpname_init (c2m_ctx_t c2m_ctx) {
  ...
  HTAB_CREATE (tpname_t, tpname_tab, memctl, 1000, tpname_hash, tpname_eq, NULL);
}
...
static void parse_init (c2m_ctx_t c2m_ctx) {
  ...
  VARR_CREATE (token_t, recorded_tokens, memctl, 32);
  VARR_CREATE (token_t, buffered_tokens, memctl, 32);
  pre_init (c2m_ctx);
...
static void symbol_init (c2m_ctx_t c2m_ctx) {
  MIR_memctl_t memctl = get_memctl (c2m_ctx);
  HTAB_CREATE_WITH_FREE_FUNC (symbol_t, symbol_tab, memctl, 5000, symbol_hash, symbol_eq, symbol_clear,
                              NULL);
}
...
static void symbol_insert (c2m_ctx_t c2m_ctx, enum symbol_mode mode, node_t id, node_t scope,
  ...
  VARR_CREATE (node_t, symbol.defs, memctl, 4);
...
static void symbol_def_replace (c2m_ctx_t c2m_ctx, symbol_t symbol, node_t def_node) {
  ...
  VARR_CREATE (node_t, defs, memctl, 4);
...
static void context_init (c2m_ctx_t c2m_ctx) {
  ...
  VARR_CREATE (node_t, context_stack, memctl, 64);
  check (c2m_ctx, n_i1_node, NULL);
  func_block_scope = curr_scope = NULL;
  VARR_CREATE (node_t, label_uses, memctl, 0);
  symbol_init (c2m_ctx);
  in_params_p = FALSE;
  curr_unnamed_anon_struct_union_member = NULL;
  HTAB_CREATE (case_t, case_tab, memctl, 100, case_hash, case_eq, NULL);
  VARR_CREATE (decl_t, func_decls_for_allocation, memctl, 1024);
  VARR_CREATE (node_t, possible_incomplete_decls, memctl, 512);
}
...
static void init_reg_vars (c2m_ctx_t c2m_ctx) {
  ...
  reg_free_mark = 0;
  HTAB_CREATE (reg_var_t, reg_var_tab, memctl, 128, reg_var_hash, reg_var_eq, NULL);
}
...
static void gen_mir_protos (c2m_ctx_t c2m_ctx) {
  ...
  HTAB_CREATE (MIR_item_t, proto_tab, memctl, 512, proto_hash, proto_eq, NULL);
...
static void gen_mir (c2m_ctx_t c2m_ctx, node_t r) {
  ...
  VARR_CREATE (MIR_var_t, proto_info.arg_vars, memctl, 32);
  VARR_CREATE (MIR_type_t, proto_info.ret_types, memctl, 16);
  gen_mir_protos (c2m_ctx);
  VARR_CREATE (MIR_op_t, call_ops, memctl, 32);
  VARR_CREATE (MIR_op_t, ret_ops, memctl, 8);
  VARR_CREATE (MIR_op_t, switch_ops, memctl, 128);
  VARR_CREATE (case_t, switch_cases, memctl, 64);
  VARR_CREATE (init_el_t, init_els, memctl, 128);
  VARR_CREATE (node_t, node_stack, memctl, 8);
...
static void init_include_dirs (c2m_ctx_t c2m_ctx) {
  ...
  VARR_CREATE (char_ptr_t, headers, memctl, 0);
  VARR_CREATE (char_ptr_t, system_headers, memctl, 0);
...
static void define_cmd_macro (c2m_ctx_t c2m_ctx, const char *name, const char *def) {
  ...
  pos.lno = 1;
  pos.ln_pos = 0;
  VARR_CREATE (token_t, repl, memctl, 16);
...
static void compile_init (c2m_ctx_t c2m_ctx, struct c2mir_options *ops, int (*getc_func) (void *),
  ...
  VARR_CREATE (char, symbol_text, memctl, 128);
  VARR_CREATE (char, temp_string, memctl, 128);
  VARR_CREATE (pos_t, node_positions, memctl, 128);
  parse_init (c2m_ctx);
  context_init (c2m_ctx);
  init_include_dirs (c2m_ctx);
  process_macro_commands (c2m_ctx);
  VARR_CREATE (node_t, call_nodes, memctl, 128); /* used in context and gen */
  VARR_CREATE (node_t, containing_anon_members, memctl, 8);
  VARR_CREATE (init_object_t, init_object_path, memctl, 8);
}
