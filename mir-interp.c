...
static void generate_icode (MIR_context_t ctx, MIR_item_t func_item) {
  ...
  func_item->data = func_desc
    = MIR_malloc (ctx->memctl, sizeof (struct func_desc) + VARR_LENGTH (MIR_val_t, code_varr) * sizeof (MIR_val_t));
  if (func_desc == NULL)
...
static void finish_func_interpretation (MIR_item_t func_item, MIR_memctl_t memctl) {
  ...
  MIR_free (memctl, func_item->data);
  func_item->data = NULL;
}
...
static void ff_interface_clear (ff_interface_t ffi, void *arg) {
  MIR_memctl_t memctl = (MIR_memctl_t) arg;
  MIR_free (memctl, ffi);
}
...
static void *get_ff_interface (MIR_context_t ctx, size_t arg_vars_num, size_t nres,
  ...
  ffi = MIR_malloc (ctx->memctl, sizeof (struct ff_interface) + sizeof (_MIR_arg_desc_t) * nargs
...
static void interp_init (MIR_context_t ctx) {
  MIR_memctl_t memctl = ctx->memctl;
  ...
  if ((interp_ctx = ctx->interp_ctx = MIR_malloc (memctl, sizeof (struct interp_ctx))) == NULL)
    MIR_get_error_func (ctx) (MIR_alloc_error, "Not enough memory for ctx");
#if DIRECT_THREADED_DISPATCH
  eval (ctx, NULL, NULL, NULL);
#endif
  VARR_CREATE (MIR_insn_t, branches, memctl, 0);
  VARR_CREATE (MIR_val_t, code_varr, memctl, 0);
  VARR_CREATE (MIR_val_t, arg_vals_varr, memctl, 0);
  arg_vals = VARR_ADDR (MIR_val_t, arg_vals_varr);
  VARR_CREATE (MIR_val_t, call_res_args_varr, memctl, 0);
  VARR_CREATE (_MIR_arg_desc_t, call_arg_descs_varr, memctl, 0);
  call_res_args = VARR_ADDR (MIR_val_t, call_res_args_varr);
  call_arg_descs = VARR_ADDR (_MIR_arg_desc_t, call_arg_descs_varr);
  HTAB_CREATE_WITH_FREE_FUNC (ff_interface_t, ff_interface_tab, memctl, 1000, ff_interface_hash,
                              ff_interface_eq, ff_interface_clear, memctl);
#if MIR_INTERP_TRACE
  trace_insn_ident = 0;
#endif
...
static void interp_finish (MIR_context_t ctx) {
  ...
  /* Clear func descs???  */
  MIR_free (ctx->memctl, ctx->interp_ctx);
  ctx->interp_ctx = NULL;
}
