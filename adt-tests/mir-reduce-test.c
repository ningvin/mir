...
int main (int argc, const char *argv[]) {
  ...
  VARR_CREATE (uint8_t, orig, memctl, 0);
  VARR_CREATE (uint8_t, buf1, memctl, 0);
  if (!reduce_encode (memctl, reader1, writer1, NULL)) {
  ...
  VARR_CREATE (uint8_t, buf2, memctl, 0);
  start = real_usec_time ();
  if (!reduce_decode (memctl, reader2, writer2, NULL)) {
