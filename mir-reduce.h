...
static inline struct reduce_data *reduce_encode_start (MIR_memctl_t memctl, reduce_writer_t writer,
                                                       void *aux_data) {
  struct reduce_data *data = MIR_malloc (memctl, sizeof (struct reduce_data));
...
static inline int reduce_encode_finish (MIR_memctl_t memctl, struct reduce_data *data) {
  int ok_p;

  _reduce_encode_buf (data);
  _reduce_hash_write (data, data->check_hash);
  ok_p = data->ok_p;
  MIR_free (memctl, data);
  return ok_p;
}

static inline struct reduce_data *reduce_decode_start (MIR_memctl_t memctl, reduce_reader_t reader,
                                                       void *aux_data) {
  struct reduce_data *data = MIR_malloc (memctl, sizeof (struct reduce_data));
...
static inline int reduce_decode_finish (MIR_memctl_t memctl, struct reduce_data *data) {
  uint8_t tag;
  int ok_p
...
static inline int reduce_encode (MIR_memctl_t memctl, reduce_reader_t reader, reduce_writer_t writer,
                                 void *aux_data) {
  size_t i, size;
  uint8_t buf[_REDUCE_WRITE_IO_LEN];
  struct reduce_data *data = reduce_encode_start (memctl, writer, aux_data);
  ...
  return reduce_encode_finish (memctl, data);
}

static inline int reduce_decode (MIR_memctl_t memctl, reduce_reader_t reader, reduce_writer_t writer,
                                 void *aux_data) {
  int c, i;
  uint8_t buf[_REDUCE_WRITE_IO_LEN];
  struct reduce_data *data = reduce_decode_start (memctl, reader, aux_data);
  ...
  return reduce_decode_finish (memctl, data);
}
