...
static void finish_func_interpretation (MIR_item_t func_item, MIR_memctl_t memctl);
...
static void check_and_prepare_insn_descs (MIR_context_t ctx) {
  ...
  VARR_CREATE (size_t, insn_nops, ctx->memctl, 0);
...
static void string_init (MIR_memctl_t memctl, VARR (string_t) * *strs, HTAB (string_t) * *str_tab) {
  string_t string = {0, {0, NULL}};

  VARR_CREATE (string_t, *strs, memctl, 0);
  VARR_PUSH (string_t, *strs, string); /* don't use 0th string */
  HTAB_CREATE (string_t, *str_tab, memctl, 1000, str_hash, str_eq, NULL);
}
...
static string_t string_store (MIR_context_t ctx, VARR (string_t) * *strs,
  ...
  if ((heap_str = MIR_malloc (ctx->memctl, str.len)) == NULL)
...
static void string_finish (MIR_memctl_t memctl, VARR (string_t) * *strs, HTAB (string_t) * *str_tab) {
  size_t i;

  for (i = 1; i < VARR_LENGTH (string_t, *strs); i++)
    MIR_free (memctl, (char *) VARR_ADDR (string_t, *strs)[i].str.s);
...
static void func_regs_init (MIR_context_t ctx, MIR_func_t func) {
  ...
  if ((func_regs = func->internal = MIR_malloc (ctx->memctl, sizeof (struct func_regs))) == NULL)
    MIR_get_error_func (ctx) (MIR_alloc_error, "Not enough memory for func regs info");
  VARR_CREATE (reg_desc_t, func_regs->reg_descs, ctx->memctl, 50);
  VARR_PUSH (reg_desc_t, func_regs->reg_descs, rd); /* for 0 reg */
  HTAB_CREATE (size_t, func_regs->name2rdn_tab, ctx->memctl, 100, name2rdn_hash, name2rdn_eq, func_regs);
  HTAB_CREATE (size_t, func_regs->hrn2rdn_tab, ctx->memctl, 10, hrn2rdn_hash, hrn2rdn_eq, func_regs);
  HTAB_CREATE (size_t, func_regs->reg2rdn_tab, ctx->memctl, 100, reg2rdn_hash, reg2rdn_eq, func_regs);
}
...
static MIR_reg_t create_func_reg (MIR_context_t ctx, MIR_func_t func, const char *name,
  ...
  if (hard_reg_name != NULL) {
    ...
    if (func_module->data == NULL)
      func_module->data = bitmap_create2 (ctx->memctl, 128);
...
static void func_regs_finish (MIR_context_t ctx MIR_UNUSED, MIR_func_t func) {
  ...
  MIR_free (ctx->memctl, func->internal);
  func->internal = NULL;
}
...
MIR_memctl_t MIR_get_memctl (MIR_context_t ctx) { return ctx->memctl; }
...
MIR_context_t _MIR_init (MIR_memctl_t memctl, MIR_gen_memctl_t gen_memctl) {
  MIR_context_t ctx;

  if (memctl == NULL)
    memctl = &default_memctl;
  if (gen_memctl == NULL)
    gen_memctl = &default_gen_memctl;

  mir_assert (MIR_OP_BOUND < OUT_FLAG);
  if ((ctx = MIR_malloc (memctl, sizeof (struct MIR_context))) == NULL)
  ...
  curr_func = NULL;
  curr_label_num = 0;
  if ((ctx->string_ctx = MIR_malloc (memctl, sizeof (struct string_ctx))) == NULL
      || (ctx->alias_ctx = MIR_malloc (memctl, sizeof (struct string_ctx))) == NULL)
    MIR_get_error_func (ctx) (MIR_alloc_error, "Not enough memory for ctx");
  string_init (memctl, &strings, &string_tab);
  string_init (memctl, &aliases, &alias_tab);
  VARR_CREATE (MIR_proto_t, unspec_protos, memctl, 0);
  check_and_prepare_insn_descs (ctx);
  DLIST_INIT (MIR_module_t, all_modules);
  simplify_init (ctx);
  VARR_CREATE (char, temp_string, memctl, 64);
  VARR_CREATE (uint8_t, temp_data, memctl, 512);
  VARR_CREATE (uint8_t, used_label_p, memctl, 512);
#if !MIR_NO_IO
  io_init (ctx);
#endif
#if !MIR_NO_SCAN
  scan_init (ctx);
#endif
  VARR_CREATE (MIR_module_t, modules_to_link, memctl, 0);
  VARR_CREATE (MIR_op_t, temp_ops, memctl, 0);
  init_module (ctx, &environment_module, ".environment");
  HTAB_CREATE (MIR_item_t, module_item_tab, ctx->memctl, 512, item_hash, item_eq, NULL);
  setjmp_addr = NULL;
  code_init (ctx);
...
static void remove_item (MIR_context_t ctx, MIR_item_t item) {
  ...
  case MIR_func_item:
    ...
    MIR_free (ctx->memctl, item->u.func);
    break;
  case MIR_proto_item:
    VARR_DESTROY (MIR_var_t, item->u.proto->args);
    MIR_free (ctx->memctl, item->u.proto);
    break;
  ...
  case MIR_data_item:
    if (item->addr != NULL && item->section_head_p)
      MIR_free (ctx->memctl, item->addr);
    MIR_free (ctx->memctl, item->u.data);
    break;
  case MIR_ref_data_item:
    if (item->addr != NULL && item->section_head_p)
      MIR_free (ctx->memctl, item->addr);
    MIR_free (ctx->memctl, item->u.ref_data);
    break;
  case MIR_lref_data_item:
    if (item->addr != NULL && item->section_head_p)
      MIR_free (ctx->memctl, item->addr);
    MIR_free (ctx->memctl, item->u.lref_data);
    break;
  case MIR_expr_data_item:
    if (item->addr != NULL && item->section_head_p)
      MIR_free (ctx->memctl, item->addr);
    MIR_free (ctx->memctl, item->u.expr_data);
    break;
  case MIR_bss_item:
    if (item->addr != NULL && item->section_head_p)
      MIR_free (ctx->memctl, item->addr);
    MIR_free (ctx->memctl, item->u.bss);
    break;
  default: mir_assert (FALSE);
  }
  if (item->data != NULL)
    MIR_free (ctx->memctl, item->data);
  MIR_free (ctx->memctl, item);
}
...
void MIR_finish (MIR_context_t ctx) {
  ...
  while (VARR_LENGTH (MIR_proto_t, unspec_protos) != 0) {
    ...
    MIR_free (ctx->memctl, proto);
  }
  VARR_DESTROY (MIR_proto_t, unspec_protos);
  string_finish (ctx->memctl, &strings, &string_tab);
  string_finish (ctx->memctl, &aliases, &alias_tab);
  simplify_finish (ctx);
  ...
  if (curr_module != NULL)
    MIR_get_error_func (ctx) (MIR_finish_error, "finish when module %s is not finished",
                              curr_module->name);
  MIR_free (ctx->memctl, ctx->string_ctx);
  MIR_free (ctx->memctl, ctx->alias_ctx);
  MIR_free (ctx->memctl, ctx);
  ctx = NULL;
}
...
MIR_module_t MIR_new_module (MIR_context_t ctx, const char *name) {
  if (curr_module != NULL)
    MIR_get_error_func (ctx) (MIR_nested_module_error,
                              ...
                              curr_module->name);
  if ((curr_module = MIR_malloc (ctx->memctl, sizeof (struct MIR_module))) == NULL)
...
static MIR_item_t create_item (MIR_context_t ctx, MIR_item_type_t item_type,
  ...
  if ((item = MIR_malloc (ctx->memctl, sizeof (struct MIR_item))) == NULL)
...
MIR_item_t MIR_new_bss (MIR_context_t ctx, const char *name, size_t len) {
  ...
  item->u.bss = MIR_malloc (ctx->memctl, sizeof (struct MIR_bss));
  if (item->u.bss == NULL) {
    MIR_free (ctx->memctl, item);
...
MIR_item_t MIR_new_data (MIR_context_t ctx, const char *name, MIR_type_t el_type, size_t nel,
  ...
  item->u.data = data = MIR_malloc (ctx->memctl, sizeof (struct MIR_data) + el_len * nel);
  if (data == NULL) {
    MIR_free (ctx->memctl, item);
...
MIR_item_t MIR_new_ref_data (MIR_context_t ctx, const char *name, MIR_item_t ref_item,
  ...
  item->u.ref_data = ref_data = MIR_malloc (ctx->memctl, sizeof (struct MIR_ref_data));
  if (ref_data == NULL) {
    MIR_free (ctx->memctl, item);
...
MIR_item_t MIR_new_lref_data (MIR_context_t ctx, const char *name, MIR_label_t label,
  ...
  item->u.lref_data = lref_data = MIR_malloc (ctx->memctl, sizeof (struct MIR_lref_data));
  if (lref_data == NULL) {
    MIR_free (ctx->memctl, item);
...
MIR_item_t MIR_new_expr_data (MIR_context_t ctx, const char *name, MIR_item_t expr_item) {
  ...
  item->u.expr_data = expr_data = MIR_malloc (ctx->memctl, sizeof (struct MIR_expr_data));
  if (expr_data == NULL) {
    MIR_free (ctx->memctl, item);
...
static MIR_proto_t create_proto (MIR_context_t ctx, const char *name, size_t nres,
                                 ...
                                 MIR_var_t *args) {
  MIR_proto_t proto = MIR_malloc (ctx->memctl, sizeof (struct MIR_proto) + nres * sizeof (MIR_type_t));
  MIR_var_t arg;
  ...
  VARR_CREATE (MIR_var_t, proto->args, ctx->memctl, nargs);
...
static MIR_item_t new_func_arr (MIR_context_t ctx, const char *name, size_t nres,
  ...
  curr_func = func_item->u.func = func
    = MIR_malloc (ctx->memctl, sizeof (struct MIR_func) + nres * sizeof (MIR_type_t));
  if (func == NULL) {
    MIR_free (ctx->memctl, func_item);
  ...
  VARR_CREATE (MIR_var_t, func->vars, ctx->memctl, nargs + 8);
...
static MIR_reg_t new_func_reg (MIR_context_t ctx, MIR_func_t func, MIR_type_t type,
  ...
  if (hard_reg_name == NULL) {
    ...
    if (func->global_vars == NULL) VARR_CREATE (MIR_var_t, func->global_vars, ctx->memctl, 8);
...
static MIR_item_t load_bss_data_section (MIR_context_t ctx, MIR_item_t item, int first_only_p) {
  ...
  if (item->addr == NULL) {
    ...
    if ((item->addr = MIR_malloc (ctx->memctl, section_size)) == NULL) {
  ...
  if (set_interface != NULL) {
    while (VARR_LENGTH (MIR_module_t, modules_to_link) != 0) {
      ...
      for (item = DLIST_HEAD (MIR_item_t, m->items); item != NULL;
           item = DLIST_NEXT (MIR_item_t, item))
        if (item->item_type == MIR_func_item) {
          finish_func_interpretation (item, ctx->memctl); /* in case if it was used for expr data */
...
static MIR_insn_t create_insn (MIR_context_t ctx, size_t nops, MIR_insn_code_t code) {
  ...
  if (nops == 0) nops = 1;
  insn = MIR_malloc (ctx->memctl, sizeof (struct MIR_insn) + sizeof (MIR_op_t) * (nops - 1));
  if (insn == NULL)
...
MIR_insn_t MIR_copy_insn (MIR_context_t ctx, MIR_insn_t insn) {
  ...
  MIR_insn_t new_insn = MIR_malloc (ctx->memctl, size);
...
void _MIR_duplicate_func_insns (MIR_context_t ctx, MIR_item_t func_item) {
  ...
  DLIST_INIT (MIR_insn_t, func->insns);
  VARR_CREATE (MIR_insn_t, labels, ctx->memctl, 0);
  VARR_CREATE (MIR_insn_t, branch_insns, ctx->memctl, 0);
...
static void simplify_init (MIR_context_t ctx) {
  if ((ctx->simplify_ctx = MIR_malloc (ctx->memctl, sizeof (struct simplify_ctx))) == NULL)
    MIR_get_error_func (ctx) (MIR_alloc_error, "Not enough memory for ctx");
  HTAB_CREATE (val_t, val_tab, ctx->memctl, 512, val_hash, val_eq, ctx);
  VARR_CREATE (MIR_insn_t, temp_insns, ctx->memctl, 0);
  VARR_CREATE (MIR_insn_t, cold_insns, ctx->memctl, 0);
  VARR_CREATE (MIR_insn_t, labels, ctx->memctl, 0);
  VARR_CREATE (MIR_reg_t, inline_reg_map, ctx->memctl, 256);
  VARR_CREATE (MIR_insn_t, anchors, ctx->memctl, 32);
  VARR_CREATE (size_t, alloca_sizes, ctx->memctl, 32);
...
static void simplify_finish (MIR_context_t ctx) {
  ...
  MIR_free (ctx->memctl, ctx->simplify_ctx);
  ctx->simplify_ctx = NULL;
}
...
static void code_init (MIR_context_t ctx) {
  if ((ctx->machine_code_ctx = MIR_malloc (ctx->memctl, sizeof (struct machine_code_ctx))) == NULL)
    MIR_get_error_func (ctx) (MIR_alloc_error, "Not enough memory for ctx");
  page_size = mem_page_size ();
  VARR_CREATE (code_holder_t, code_holders, ctx->memctl, 128);
}
...
static void code_finish (MIR_context_t ctx) {
  ...
  MIR_free (ctx->memctl, ctx->machine_code_ctx);
  ctx->machine_code_ctx = NULL;
}
...
void MIR_write_module_with_func (MIR_context_t ctx, int (*const writer) (MIR_context_t, uint8_t),
  ...
  if ((io_reduce_data = reduce_encode_start (ctx->memctl, reduce_writer, ctx)) == NULL)
  ...
  string_init (ctx->memctl, &output_strings, &output_string_tab);
  ...
  string_finish (ctx->memctl, &output_strings, &output_string_tab);
#ifndef MIR_NO_BIN_COMPRESSION
  if (!reduce_encode_finish (ctx->memctl, io_reduce_data))
...
void MIR_read_with_func (MIR_context_t ctx, int (*const reader) (MIR_context_t)) {
  ...
  io_reader = reader;
#ifndef MIR_NO_BIN_COMPRESSION
  if ((io_reduce_data = reduce_decode_start (ctx->memctl, reduce_reader, ctx)) == NULL)
    MIR_get_error_func (ctx) (MIR_binary_io_error,
...
#ifndef MIR_NO_BIN_COMPRESSION
  reduce_decode_finish (ctx->memctl, io_reduce_data);
#endif
}
...
static void io_init (MIR_context_t ctx) {
  ...
  if ((ctx->io_ctx = MIR_malloc (ctx->memctl, sizeof (struct io_ctx))) == NULL)
    MIR_get_error_func (ctx) (MIR_alloc_error, "Not enough memory for ctx");
  VARR_CREATE (MIR_var_t, proto_vars, ctx->memctl, 0);
  VARR_CREATE (MIR_type_t, proto_types, ctx->memctl, 0);
  VARR_CREATE (MIR_op_t, read_insn_ops, ctx->memctl, 0);
  VARR_CREATE (MIR_str_t, bin_strings, ctx->memctl, 512);
  VARR_CREATE (uint64_t, insn_label_string_nums, ctx->memctl, 64);
  VARR_CREATE (MIR_label_t, func_labels, ctx->memctl, 512);
}
...
static void io_finish (MIR_context_t ctx) {
  ...
  MIR_free (ctx->memctl, ctx->io_ctx);
  ctx->io_ctx = NULL;
}
...
static void scan_init (MIR_context_t ctx) {
  ...
  if ((ctx->scan_ctx = MIR_malloc (ctx->memctl, sizeof (struct scan_ctx))) == NULL)
    MIR_get_error_func (ctx) (MIR_alloc_error, "Not enough memory for ctx");
  VARR_CREATE (char, error_msg_buf, ctx->memctl, 0);
  VARR_CREATE (MIR_var_t, scan_vars, ctx->memctl, 0);
  VARR_CREATE (MIR_type_t, scan_types, ctx->memctl, 0);
  VARR_CREATE (MIR_op_t, scan_insn_ops, ctx->memctl, 0);
  VARR_CREATE (label_name_t, label_names, ctx->memctl, 0);
  HTAB_CREATE (label_desc_t, label_desc_tab, ctx->memctl, 100, label_hash, label_eq, NULL);
  HTAB_CREATE (insn_name_t, insn_name_tab, ctx->memctl, MIR_INSN_BOUND, insn_name_hash, insn_name_eq, NULL);
  for (i = 0; i < MIR_INSN_BOUND; i++) {
    in.code = i;
...
static void scan_finish (MIR_context_t ctx) {
  ...
  MIR_free (ctx->memctl, ctx->scan_ctx);
  ctx->scan_ctx = NULL;
}
...
static void hard_reg_name_init (MIR_context_t ctx) {
  ...
  if ((ctx->hard_reg_ctx = MIR_malloc (ctx->memctl, sizeof (struct hard_reg_ctx))) == NULL)
    MIR_get_error_func (ctx) (MIR_alloc_error, "Not enough memory for ctx");
  HTAB_CREATE (hard_reg_desc_t, hard_reg_desc_tab, ctx->memctl, 200, hard_reg_desc_hash, hard_reg_desc_eq, NULL);
...
static void hard_reg_name_finish (MIR_context_t ctx) {
  ...
  MIR_free (ctx->memctl, ctx->hard_reg_ctx);
  ctx->hard_reg_ctx = NULL;
}
